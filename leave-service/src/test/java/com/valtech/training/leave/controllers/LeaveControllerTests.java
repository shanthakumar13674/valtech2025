...
class LeaveControllerTests {
	...
	void testApplyAndApprove()	{
		...
		LeaveVO vo = restTemplate.postForObject(url+"apply",new LeaveVO(0,"08-03-2025","09-03-2025"
			,LeaveType.CASUAL.name(),LeaveStatus.APPLIED.name(),"Going to Native","",7), LeaveVO.class);
		assertTrue(vo.id() != 0);
	...
	void test() {
		...
		if(leaves.size() == 0) {
			//	6,7,8,9,10 valid emps
			restTemplate.postForObject(url, new LeaveVO(0,"08-03-2025","09-03-2025"
		,LeaveType.CASUAL.name(),LeaveStatus.APPLIED.name(),"Going to Native","",6), LeaveVO.class);
			restTemplate.postForObject(url, new LeaveVO(0,"12-03-2025","13-03-2025"
		,LeaveType.SICK.name(),LeaveStatus.APPLIED.name(),"Have Fever","",6), LeaveVO.class);
			restTemplate.postForObject(url, new LeaveVO(0,"08-03-2025","09-03-2025"
		,LeaveType.CASUAL.name(),LeaveStatus.APPLIED.name(),"Going to Native","",7), LeaveVO.class);
		}
	}
